...
Current phase: **Phase 3A.7 - CompleteEASE source recovery + 3L2 provenance lock**.
...
- **Phase 3A.8 - Source Candidate Triage Or Relative-Only Diagnostic Policy.**
  Planning: `high` for choosing between source triage and a relative-only
  diagnostic lane; `xhigh` before approving any threshold or calibrated-evidence
  policy. Implementation: `medium` for source triage/reporting, `high` if
  contract or claim-gate logic changes.
